...
const EditInfo = () => {
  ...
  const maskCpf = (cpfValue) => {
    ...
    // Mostra apenas os últimos 3 dígitos antes do traço: ***.***.820-90
    const lastThreeBeforeCheck = cpfValue.slice(-5, -2);
    const verification = cpfValue.slice(-2);
    return `***.***.${lastThreeBeforeCheck}-${verification}`;
  };
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        setFields({
          username: "",
          fullName: perfilResponse.nomeCompleto || "",
          email: "",
          phone: perfilResponse.telefone || "",
        });
        setCpf(perfilResponse.cpf || "");
      } catch (error) {
          ...
          <div className="md:col-span-2">
            <div className="rounded-xl border border-[#d1e1f0] bg-[#f5f9ff] px-6 py-4 text-center">
              ...
              <p className="mt-2 font-mono text-lg tracking-widest">
                {fields.fullName || "***********"} / {maskCpf(cpf)} / **-10-90
              </p>
